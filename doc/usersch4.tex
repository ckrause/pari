...
\fun{GEN}{gc_GEN}{pari_sp ltop, GEN x}. Functionally equivalent to,
...
\kbd{gc\_GEN}, this is the most robust of the \kbd{gerepile} functions
(the less prone to user error), hence the slowest. For convenience,
this function also returns the new value of $x$.

\fun{void}{gc_all_unsafe}{pari_sp ltop, pari_sp lbot, int n, ...}.
More efficient, but trickier than \kbd{gc\_all}. Cleans the stack between
...
Let us now see why we may need the \kbd{gc\_all} variants. Although it
...
address of \kbd{z2}. This is where we need the \idx{gc_all}
function:
\bprog
   gc_all(ltop, 2, &z1, &z2)
@eprog
...
\kbd{gc\_all\_unsafe} instead, which is usually faster since we do not need
...
lost, just use \tet{gc_GEN} or \tet{gc_all} to fix up the stack
