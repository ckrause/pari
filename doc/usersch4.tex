...
PARI functions. This is at most twice slower than \kbd{gc\_upto}. Again
\kbd{oldz} should not be accessed anymore.
...
the underlying mechanisms in terms of low level functions. The most
fundamental (and fastest) one is

\fun{GEN}{gc_GEN_unsafe}{pari_sp ltop, pari_sp lbot, GEN q}. This function
cleans up the stack between \kbd{ltop} and \kbd{lbot}, where $\kbd{lbot} <
...
this is checked in the function. But beware as well that the addresses
of the objects in the translated zone have now changed,
...
\kbd{gc\_all} function below.

\misctitle{Remark}
As a consequence of the preceding explanation, if a PARI object is to be
relocated by \tet{gc_GEN_unsafe} then, apart from universal objects, the chunks
...
this. This is because that function knows only about \emph{two
...
\bprog
  ...
  return gc_GEN_unsafe(ltop, lbot, q); /*@Ccom returns the updated q */
@eprog\noindent or directly
\bprog
  ltop = avma; garbage(); lbot = avma;
  return gc_GEN_unsafe(ltop, lbot, anything());
@eprog\noindent
Beware that
\bprog
  ltop = avma; garbage();
  return gc_GEN_unsafe(ltop, avma, anything())
@eprog

\noindent is incorrect. We cannot predict whether
\kbd{avma} is evaluated after or before the call to \kbd{anything()}: it
depends on the compiler. If we are out of luck, it is evaluated
\emph{after} the call and the result belongs to the garbage zone.
Thus we return a pointer to random garbage.

\misctitle{Remark on assignments} When the tree structure
and the size of the PARI objects which will appear in a computation are under
control, one may allocate sufficiently large objects at the beginning, use
assignment statements, then simply restore \kbd{avma}. Coming back to the
...
\kbd{gc\_GEN\_unsafe} at all:
\bprog
  z = cgetr(DEFAULTPREC); ltop = avma;
  gaffect(gadd(gsqr(x), gsqr(y)), z);
  set_avma(ltop);
@eprog\noindent This is often \emph{slower} than garbage collection, and
certainly more cumbersome to use. As a rule, assignment statements using
\kbd{gaffect} and variants should be avoided.

\subsec{The basic 3 garbage collecting functions}

Let us recap: although all is implemented in terms of the
\kbd{gc\_GEN\_unsafe} low level function, you don't want to use it directly.
You mostly want to use one of the following three functions to collect
garbage while keeping some results safe.
...
Low level PARI functions (such as shallow functions) may not satisfy this:
this will be mentioned in their documentation. The technical condition is as
follows: \kbd{q} must be connected and have been created \emph{before} all
its components, otherwise they would belong to the garbage zone!

\fun{GEN}{gc_GEN}{pari_sp ltop, GEN x}. Works in all cases. Functionally
equivalent to, but more efficient than
\bprog
  gc_upto(ltop, gcopy(x))
...
twice slower than either \tet{gc_upto} or \tet{gc_GEN_unsafe}, because
\kbd{x} has to be copied to a clean stack zone first. This function is a
special case of \tet{gc_all} below, where $n=1$.
...
\kbd{gc\_GEN}, this is the most robust of the garbage collecting functions
(the less prone to user error), hence the slowest. For convenience,
this function also returns the new value of $x$.
...
Using \tet{gc_GEN_unsafe}, we can give back the memory locations
...
\bprog
  ...
  z = gc_GEN_unsafe(ltop, lbot, z);
@eprog
\noindent Of course, the last two instructions could also have been
written more simply:
\bprog
  z = gc_GEN_unsafe(ltop, lbot, gadd(x2,y2));
@eprog\noindent Finally, the best solution is to use \kbd{gc\_upto}, because
the result of \kbd{gadd} is the last object on the stack (and \kbd{gadd}
is not a shallow function):
\bprog
  ltop = avma;
  z = gc_upto(ltop, gadd(gsqr(x), gsqr(y)));
@eprog\noindent Another possibility is to use
\bprog
  ltop = avma;
...
negligible compared to the squaring times.
...
better. The only condition for \tet{gc_GEN_unsafe} to work is that the
...
\bprog
  ...
  z = cgetg(3, t_VEC);
  gel(z, 1) = gadd(x2,y);
  gel(z, 2) = gadd(y2,x); z = gc_upto(ltop,lbot,z);
@eprog\noindent Beware that
\bprog
...
\bprog
  ltop = avma; x2 = gsqr(x); y2 = gsqr(y);
  lbot = avma; z = mkvec2(gadd(x2,y), gadd(y2,x));
  z = gc_GEN_unsafe(ltop,lbot,z);
@eprog\noindent but is subtly wrong in the sense that
\kbd{z = gc\_upto(ltop, z)} would \emph{not} work. The reason being
...
too early; \tet{gc_GEN_unsafe} does not care, but the created \kbd{z} is a time
bomb which will explode on any later \kbd{gc\_upto}.
On the other hand
\bprog
...
lets \kbd{gc\_upto} update them for naught). Finally, the most elegant
...
gel(z, 1) = gsub(a,b);
gel(z, 2) = gsub(c,d); z = gc_upto(ltop,lbot,z);
@eprog
...
to a consistent PARI programming style. At first, just use \tet{gc_GEN} or
\tet{gc_all} to fix up the stack for you. You can always optimize later when
...
paragraph is inefficient and often awkward: one of the garbage collection
...
garbage collection, and about as efficient.
