...
garbage while keeping some results safe. Many others will be described in
\secref{se:garbage2} for specialized scenarios.
...
twice slower than \tet{gc_upto}, because \kbd{q} has to be copied to a clean
stack zone first. This function is a special case of \tet{gc_all} below,
where $n=1$.
...
Using \tet{gc_upto}, we can give back the memory locations
...
object on the stack and everything else is deleted. Another possibility is
to use
\bprog
  ltop = avma;
...
better. The only condition for \tet{gc_upto} to work is that the
