...
suitable for \tet{gc_upto}, the function description will state it. Of course
\tet{gc_GEN} and \tet{gc_all} are always able to handle it.
...
\fun{GEN}{gc_GEN_unsafe}{pari_sp p, pari_sp q, GEN x} low level garbage
...
\fun{GEN}{gc_INT}{pari_sp av, GEN q} faster variant of \kbd{gc\_GEN} when
\kbd{q} is a \typ{INT}. The length and effective length of the resulting
\typ{INT} are equal.
...
variant of \tet{gc_all}, about twice faster. This is subject to the same
restrictions as \tet{gc_GEN_unsafe}, the only difference being that more than
...
\bprog
  ...
  z = cgetg(3, t_COMPLEX);
  z[1] = fun1();
  z[2] = fun2(); return gc_GEN_unsafe(av, ltop, z);
@eprog\noindent
...
\fun{GEN}{gc_uptoleaf}{pari_sp av, GEN q} analogous to (but faster than)
\kbd{gc\_upto}. Assumes further that \kbd{q} is a leaf, i.e a
nonrecursive type (\kbd{is\_recursive\_t(typ(q))} is nonzero). Contrary to
\kbd{gc\_INT} and \kbd{gc\_upto}, \kbd{gc\_uptoleaf} leaves
...
\tet{gc_GEN_unsafe} will not inspect their components, saving time.
...
\fun{GEN}{Flx_translate}{GEN P, ulong c, ulong p} return $P(x+c)$, $p$ must be
...
\fun{GEN}{FlxC_translate}{GEN x, ulong c, ulong p}
...
\fun{GEN}{FlxM_translate}{GEN x, ulong c, ulong p}
...
form $Q(X^d)$, return $Q$. Shallow function.
...
\fun{GEN}{RgX_homogenize_deg}{GEN P, long d, long v} Return the homogenous polynomial
...
\kbd{avma}. But you will be better off not using them.
...
a priori suitable for \tet{gc_upto}. In particular, they are
...
universal objects, but nonetheless suitable for \tet{gc_upto}.
